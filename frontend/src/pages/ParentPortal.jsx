            ...
            <div className="divide-y divide-gray-100 flex-1 flex flex-col" style={{ divideColor: '#f3f4f6' }}>
              {group.vaccines.map((v) => {
                ...
                return (
                  ...
                  >
                    <span 
                      className="text-[8.5px] font-bold break-words pr-1 leading-tight flex-1"
...
const VaccinationCertificate = ({ patient, vaccinations, onVaccineClick }) => {
  ...
  console.log("VaccinationCertificate (Preview) received vaccinations in ParentPortal:", fullVaccinations);
...
const ParentPortal = () => {
  ...
  const loadPortalData = async () => {
    ...
    try {
      ...
      const [patientRes, summaryRes, vaccRes] = await Promise.all([
        ...
        axios.get(`${API_URL}/patients/${patientId}/vaccination-summary`),
        axios.get(`${API_URL}/patients/${patientId}/vaccinations`)
      ]);
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Banner */}
      <div className="bg-surface border border-border p-6 rounded-lg shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-accent text-white rounded-full flex items-center justify-center shadow-sm">
            <Activity className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-2xl font-extrabold text-primary tracking-tight">KIDDOSCARE Parent Portal</h1>
            <p className="text-sm text-secondary font-semibold mt-0.5">
              Secure patient immunization portal for <span className="text-accent">{patient?.namePrefix} {patient?.name}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-surface border border-border p-6 rounded-lg shadow-sm flex flex-col justify-center">
          ...
          <span className="text-xs text-secondary mt-2">Doses administered</span>
        </div>
        <div className="bg-surface border border-border p-6 rounded-lg shadow-sm flex flex-col justify-center">
          ...
          <span className="text-xs text-secondary mt-2">Remaining pending doses</span>
        </div>
        <div className="bg-surface border border-border p-6 rounded-lg shadow-sm flex flex-col justify-center">
